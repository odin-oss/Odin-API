...
/**
 * Particular data to customize a template for a specific environment.
 */
export class AgentTemplateGeneric {
    #environment_label;

    constructor(props) {
        const data = Guard.validateProps(AgentTemplateGeneric.schema, props);
        this.#environment_label = data.environment_label;
    }

    // Zod Schema for object validation
    static schema = z.object({
        environment_label: z.string().min(2).default('N/A'),
    });

    toJSON() {
        return {
            environment_label: this.#environment_label,
        };
    }
}
...
export class AgentTemplate {
    ...
    static schema = z.object({
        ...
        template: z.instanceof(AgentTemplateGeneric).default(new AgentTemplateGeneric({ environment_label: 'N/A' })),
    });
