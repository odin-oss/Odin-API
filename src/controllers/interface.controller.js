  ...
  await fns
    ...
    .then((response) =>
      ApiResponse.success(
        req,
        res,
        response.map((inter) => inter.toJSON()),
        200,
        'List of interfaces transmitted.'
      )
    )
    .catch((error) => {
      logs.debug(error);
...
export const create = async function (
  ...
  try {
    ...
    await fns
      ...
      .then((inter) =>
        ApiResponse.success(req, res, inter.toJSON(), 200, 'Interface created.')
      );
  } catch (err) {
    logs.debug(err);
...
export const update = async function (
  ...
  try {
    ...
    await fns
      .update({ ...body_data, id_interface: params_data.id_interface })
      .then((inter) =>
        ApiResponse.success(req, res, inter.toJSON(), 200, 'Interface updated.')
      );
  } catch (err) {
    logs.debug(err);
    ApiResponse.error(req, res, err);
